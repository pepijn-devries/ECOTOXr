...
search_results <- websearch_ecotox(
  search_fields, ssl_verifypeer = FALSE, ssl_verifyhost = FALSE)

search_results$`Aquatic-Export`
```

When aiming for reproducibility it is better to query the local database, as the online
tool may be able to search and export all fields; data may change and is beyond your control;
and it is processed on the server which forms a black box.